...
export default function AddDataModal({ isOpen, onClose }: AddDataModalProps) {
  ...
  const handleBatterySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Data penggantian baterai berhasil ditambahkan!');
    } catch (error) {
      console.error('Error adding battery replacement:', error);
      alert('Gagal menambahkan data penggantian baterai. Silakan coba lagi.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleMaintenanceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Gagal menambahkan data maintenance. Silakan coba lagi.');
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 p-6 text-white">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-emerald-100 text-sm mt-1">{getTabTitle()}</p>
            </div>
            <button 
              onClick={handleClose}
